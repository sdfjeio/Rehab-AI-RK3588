...
// DFL bbox decoding for official 4-output model (INT8 path)
...
// DFL bbox decoding for official 4-output model (FP16/non-quantized path)
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold,
    ...
    // Official model has 4 outputs: 3 bbox (per stride) + 1 keypoint
    // output[0]: stride 8  (80x80)
    // output[1]: stride 16 (40x40)
    // output[2]: stride 32 (20x20)
    // output[3]: keypoints (51 channels x 8400 spatial, FP16 for INT8 model, FP32 for FP16 model)
    for (int i = 0; i < 3; i++) {
    ...
    for (int i = 0; i < validCount; ++i) {
        ...
        // Keypoints from output[3]: 51 channels = 17 keypoints x 3 coords (x,y,conf)
        // Layout: channel-major [channel * 8400 + position]
        for (int j = 0; j < 17; ++j) {
            if (app_ctx->is_quant) {
                // INT8 model: bbox tensors are INT8, keypoint tensor is FP16
                od_results->results[last_count].keypoints[j][0] =
                    ((float)((rknpu2::float16 *)_outputs[3].buf)[j * 3 * 8400 + 0 * 8400 + keypoints_index]
                     - letter_box->x_pad) / letter_box->scale;
                od_results->results[last_count].keypoints[j][1] =
                    ((float)((rknpu2::float16 *)_outputs[3].buf)[j * 3 * 8400 + 1 * 8400 + keypoints_index]
                     - letter_box->y_pad) / letter_box->scale;
                od_results->results[last_count].keypoints[j][2] =
                    (float)((rknpu2::float16 *)_outputs[3].buf)[j * 3 * 8400 + 2 * 8400 + keypoints_index];
            } else {
                // FP16 model with want_float=true: all outputs are float32
                od_results->results[last_count].keypoints[j][0] =
                    (((float *)_outputs[3].buf)[j * 3 * 8400 + 0 * 8400 + keypoints_index]
                     - letter_box->x_pad) / letter_box->scale;
                od_results->results[last_count].keypoints[j][1] =
                    (((float *)_outputs[3].buf)[j * 3 * 8400 + 1 * 8400 + keypoints_index]
                     - letter_box->y_pad) / letter_box->scale;
                od_results->results[last_count].keypoints[j][2] =
                    ((float *)_outputs[3].buf)[j * 3 * 8400 + 2 * 8400 + keypoints_index];
            }
        }
